...
typedef struct comms_t {
    ...
    void start_senders() {
        ...
        for (uint32_t index=0; index<this->conf_.send_thread_count; index++) {
            this->senders_.emplace_back(new Sender(this->send_queue_, this->reap_queue_, this->catch_queue_, this->release_queue_, this->end_points_, this->conf_.send_batch_dequeue));
        }
    }
...
int comms_put_reap(comms_t *C, comms_packet_t *packet) {
    C->reap_queue_->enqueue(packet);
    return 0;
}
